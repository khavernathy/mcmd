...
int main(int argc, char **argv) {
	...
	for (int t=0; t <= (finalstep-system.constants.step_offset); t+=stepsize) { // 0 is initial step
        ...
        if (t==0 || t % corrtime == 0 || t == finalstep) { /// output every x steps
            ...
            system.constants.time_elapsed = (std::chrono::duration_cast<std::chrono::microseconds>(end - system.constants.begin_steps).count()) /1000000.0;
            system.constants.sec_per_step = system.constants.time_elapsed/t;
            double progress = (((float)t)/finalstep*100);
            double ETA = ((system.constants.time_elapsed*finalstep/t) - system.constants.time_elapsed)/60.0;
